...
namespace Integration.Orchestrator.Backend.Api.Controllers.v1.Administration
{
    ...
    public class IntegrationController(IMediator mediator) : Controller
    {
        ...
        {
            return Ok(await _mediator.Send(new CreateIntegrationCommandRequest(new IntegrationBasicInfoRequest<IntegrationCreateRequest>(request))));
        }

        [HttpPut]
        public async Task<IActionResult> Update(IntegrationUpdateRequest request, Guid id)
        {
            return Ok(await _mediator.Send(new UpdateIntegrationCommandRequest(new IntegrationBasicInfoRequest<IntegrationUpdateRequest>(request), id)));
        }

        [HttpDelete]
        public async Task<IActionResult> Delete(Guid id)
        {
            return Ok(await _mediator.Send(new DeleteIntegrationCommandRequest(new IntegrationDeleteRequest { Id = id })));
        }

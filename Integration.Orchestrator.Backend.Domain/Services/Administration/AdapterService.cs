...
namespace Integration.Orchestrator.Backend.Domain.Services.Administration
{
        ...
        : IAdapterService<AdapterEntity>
    {
        ...
        public async Task<IEnumerable<AdapterEntity>> GetByTypeAsync(string type)
        {
            var specification = AdapterSpecification.GetByTypeExpression(type);
        ...
        private async Task ValidateBussinesLogic(AdapterEntity adapter, bool create = false) 
        {
            if (create)
            {
                ...
                {
                    throw new ArgumentException(AppMessages.Domain_AdapterExists);
                }
            }
        }
    }

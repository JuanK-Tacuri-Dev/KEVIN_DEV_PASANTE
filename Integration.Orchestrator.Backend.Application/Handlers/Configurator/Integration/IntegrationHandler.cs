...
using static Integration.Orchestrator.Backend.Application.Handlers.Configurator.Integration.IntegrationCommands;
        ...
        IRequestHandler<GetAllPaginatedIntegrationCommandRequest, GetAllPaginatedIntegrationCommandResponse>
    {
        ...
        public async Task<UpdateIntegrationCommandResponse> Handle(UpdateIntegrationCommandRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var relationConectionActive = await _synchronizationService
                    .GetByIntegrationIdAsync(integrationMap.id, await _synchronizationStatesService.GetStatusIdSyncronizationAsync());
                        ...
                        if (processFound != null && !await _statusService.GetStatusIsActiveAsync(processFound.status_id))
                        {
                            throw new OrchestratorArgumentException(string.Empty,
                                new DetailsArgumentErrors
                                {
                                    ...
                                    Description = ResponseMessageValues
                                    .GetResponseMessage(ResponseCode.NotActivatedDueToInactiveRelationship, "Proceso"),
                                    Data = request.Integration
                                });
                        }
                    }
